...
columns = ["age", "sex", "cp", "trtbps", "chol", "fbs", "restecg", "thalachh", "exng", "oldpeak", "slp", "thall"]
...
print("linear")
print(classification_report(y_test, predicted_linear, labels=[0, 1]))
print(f"F1_Score: {f1_score(y_test, predicted_linear)}")
print(f"Jaccard: {jaccard_score(y_test, predicted_linear)}")
cm = confusion_matrix(y_test, predicted_linear, labels=model_linear.classes_)
...
model_poly = SVC(kernel="poly", C=0.5)
model_poly.fit(x_train, y_train)

predicted_poly = model_poly.predict(x_test)

print("poly")
print(classification_report(y_test, predicted_poly, labels=[0, 1]))
print(f"F1_Score: {f1_score(y_test, predicted_poly)}")
print(f"Jaccard: {jaccard_score(y_test, predicted_poly)}")
cm = confusion_matrix(y_test, predicted_poly, labels=model_poly.classes_)
...
model_rbf = SVC(kernel="rbf", C=0.5)
model_rbf.fit(x_train, y_train)

predicted_rbf = model_rbf.predict(x_test)

print("rbf")
print(classification_report(y_test, predicted_rbf, labels=[0, 1]))
print(f"F1_Score: {f1_score(y_test, predicted_rbf)}")
print(f"Jaccard: {jaccard_score(y_test, predicted_rbf)}")
cm = confusion_matrix(y_test, predicted_rbf, labels=model_rbf.classes_)
...
model_sigmoid = SVC(kernel="sigmoid", C=0.5)
model_sigmoid.fit(x_train, y_train)

predicted_sigmoid = model_sigmoid.predict(x_test)

print("sigmoid")
print(classification_report(y_test, predicted_sigmoid, labels=[0, 1]))
print(f"F1_Score: {f1_score(y_test, predicted_sigmoid)}")
print(f"Jaccard: {jaccard_score(y_test, predicted_sigmoid)}")
cm = confusion_matrix(y_test, predicted_sigmoid, labels=model_sigmoid.classes_)
